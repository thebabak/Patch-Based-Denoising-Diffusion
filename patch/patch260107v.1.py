...
# Robust version: detects + fixes NaN/Inf in .npy datasets (common reason for nan loss)
#
# IMPORTANT FIXES INCLUDED:
# 1) Avoid huge RAM spike when computing global mean/std (streaming stats, no big concatenation).
# 2) Sampling correctness: "ddpm" now requires steps == T (true DDPM). For fewer steps use "ddim".
# 3) Cleaned argparse for train_on_p2 (default True with --no_train_on_p2 switch).
...
def save_epoch_sample(full_tensor: torch.Tensor, outdir: str, name: str):
    ...
    print(f" Saved sample: {npy_path} and {png_path}")
...
def _accumulate_running_stats(sum_x: float, sum_x2: float, n: int, arr: np.ndarray) -> Tuple[float, float, int]:
    # arr: (N,H,W) float32
    finite = np.isfinite(arr)
    if not finite.any():
...
class NPYGroupDataset(Dataset):
    ...
    def __init__(self, npy_paths: Dict[str, str], fix_nonfinite: bool = True):
        ...
        for cls, p in npy_paths.items():
            ...
            if first_hw is None:
                first_hw = (int(arr.shape[1]), int(arr.shape[2]))
            else:
                if (int(arr.shape[1]), int(arr.shape[2])) != first_hw:
                    raise ValueError(
                        f"All classes must share same H,W. Got {arr.shape[1:]} but expected {first_hw} for file {p}"
                    )
            ...
            # accumulate stats (finite only)
            sum_x, sum_x2, n = _accumulate_running_stats(sum_x, sum_x2, n, arr)

            self.per_class_counts[cls] = int(arr.shape[0])
            y = self.class_to_idx[cls]
            for i in range(arr.shape[0]):
                self.items.append((arr[i], y))

        if n == 0:
            self.mean, self.std = 0.0, 1.0
            print("WARNING: all values are non-finite. Using mean=0 std=1")
        else:
            mean = sum_x / n
...
def patch_aug(
    ...
    if allow_flip and torch.rand(()) < 0.5:
    ...
    if allow_noise and torch.rand(()) < p_noise:
...
class PatchPairDataset(Dataset):
    def __init__(
        ...
        self.enable_rotflip = enable_rotflip
    ...
    def __getitem__(self, _):
        ...
        if self.enable_aug:
            p1 = patch_aug(
                p1,
                allow_flip=self.enable_rotflip,
                allow_rot=self.enable_rotflip,
...
class SinTimeEmb(nn.Module):
    ...
    def forward(self, t: torch.Tensor):
        half = self.dim // 2
        freqs = torch.exp(-math.log(10000) * torch.arange(0, half, device=t.device) / max(1, (half - 1)))
...
class SmallUNet(nn.Module):
    ...
    def forward(self, x, t, y=None):
        te = self.t(t)
        ye = self.y(y) if self.use_class and (y is not None) else None
...
# Sampling (patch -> full)
# ----------------------------
def make_ddim_timesteps(T: int, steps: int, device):
    if steps <= 0:
        raise ValueError("steps must be >= 1")
    steps = min(int(steps), int(T))
    ts = torch.linspace(0, T - 1, steps, device=device)
    ts = torch.round(ts).long()
    ts = torch.unique(ts)
    if ts.numel() == 0 or ts[0].item() != 0:
        ts = torch.cat([torch.tensor([0], device=device), ts])
    if ts[-1].item() != T - 1:
        ts = torch.cat([ts, torch.tensor([T - 1], device=device)])
    return ts


@torch.no_grad()
def sample_patch_ddpm(model, diff, patch: int, in_ch: int, steps: int, device, y=None, coord=None):
    # TRUE DDPM requires all steps (no skipping). Use ddim for fewer steps.
    if steps != diff.cfg.T:
        raise ValueError(
            f"DDPM sampling requires steps == T ({diff.cfg.T}). "
            f"You set steps={steps}. Use --sample_mode ddim for fewer steps."
        )
    ...
    for t in reversed(range(diff.cfg.T)):
...
@torch.no_grad()
def sample_patch_ddim(model, diff, patch: int, in_ch: int, steps: int, device, y=None, coord=None, eta: float = 0.0):
    x_img = torch.randn((1, 1, patch, patch), device=device)
    if coord is not None:
        x = torch.cat([x_img, coord.unsqueeze(0)], dim=1)
    else:
    ...
    timesteps = make_ddim_timesteps(diff.cfg.T, steps, device).flip(0)
    for i, t in enumerate(timesteps):
        tt = torch.tensor([int(t.item())], device=device, dtype=torch.long)
        eps = model(x, tt, y=y)

        a_t = diff.alphas_cumprod[tt].view(1, 1, 1, 1)
        if i + 1 < timesteps.numel():
            t_prev = timesteps[i + 1]
            t_prev = torch.tensor([int(t_prev.item())], device=device, dtype=torch.long)
            a_prev = diff.alphas_cumprod[t_prev].view(1, 1, 1, 1)
        else:
            a_prev = torch.ones_like(a_t)

        xt = x[:, :1]
        x0 = (xt - torch.sqrt(1 - a_t) * eps) / (torch.sqrt(a_t) + 1e-8)
        ...
        xt_prev = torch.sqrt(a_prev) * x0 + dir_term + sigma * noise

        if coord is not None:
            x = torch.cat([xt_prev, x[:, 1:]], dim=1)
        else:
            x[:, :1] = xt_prev
...
@torch.no_grad()
def sample_full_image(
    model, diff, H, W, patch, stride, in_ch, steps, device,
    y=None, use_coords=True, sample_mode="ddim", ddim_eta: float = 0.0
):
    if sample_mode not in {"ddpm", "ddim"}:
    ...
    for top in range(0, H - patch + 1, stride):
        for left in range(0, W - patch + 1, stride):
            ...
            if sample_mode == "ddim":
                p = sample_patch_ddim(model, diff, patch, in_ch, steps, device, y=y, coord=coord, eta=ddim_eta)
            else:
...
def train(args):
    ...
    ds = PatchPairDataset(
        ...
        enable_rotflip=not args.no_rotflip_aug,
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            if not torch.isfinite(loss):
                ...
                print("WARNING Non-finite loss detected. Skipping this step.")
...
def parse_args():
    ...
    # train_on_p2: default True (same as your original intent), with an explicit disable switch.
    ap.add_argument("--no_train_on_p2", dest="train_on_p2", action="store_false")
    ap.set_defaults(train_on_p2=True)
    ...
    ap.add_argument("--no_aug", action="store_true", default=False)
    ap.add_argument("--no_rotflip_aug", action="store_true", default=False)
    ...
    ap.add_argument("--sample_mode", type=str, default="ddim", choices=["ddpm", "ddim"])
    ...
    # fix dataset NaN/Inf + stop if too many bad steps
